...
- [x] Inspect the floor blend/depth/shader paths and current cover/frame materials.
- [x] Increase stable base visibility while retaining transparency; add subtle surface panel seams and broad reflected highlights on the glass.
- [x] Give cover and frame readable bevels/edges/material contrast using batched geometry and stable scene resources.
- [x] Refine planet/cloud/noise scales and atmosphere to remove the repetitive mottled appearance. Keep the background quiet around the stage.
- [x] Retain quality gates and lifecycle disposal. Write regression assertions for finite deck parts, material readiness, map transitions and no resource growth, without executing yet.
- [x] Commit this isolated surface and return its exact head and review notes.
...
- [x] Trace shared material creation and clone adoption before changing finish maps.
- [x] Author distinct brushed steel, matte graphite and painted armor response; reduce broad self-illumination while preserving visible colored accents and legacy lighting-off behavior.
- [x] Refine torso/shoulder/foot stance and weight transfer in existing motion functions; keep exact attack contact poses and no accumulating transforms.
- [x] Keep all 25 forms and cosmetics compatible; avoid increasing mesh counts without a visible purpose.
- [x] Update meaningful material/motion regressions for the new requirements without executing yet.
- [x] Commit and return exact head, changed seams and verification that remains pending.
...
- [x] Inspect live wide/close screenshots and current CSS cascade.
- [x] Increase low-contrast text clarity, simplify visual framing, reduce heavy panel treatment and emphasize controls needed while spectating.
- [x] Keep touch targets and keyboard focus visible; preserve drawer, consent, chat and error surfaces.
- [x] Preserve reduced-motion overrides and avoid new animation or decorative assets.
- [x] Commit and return exact head and any needed coordinator markup changes.
...
- [x] Establish shared exposure/contrast/vignette bases used both at initialization and after dynamic grading resets.
- [x] Tune existing key/fill/rim directions and colors for readable front/side surfaces without adding a shadow pass.
- [x] Implement stable combat framing: choose a nearby living pair, retain focus for a dwell interval, use hysteresis before switching and restore manual pan/zoom control immediately.
- [x] Reduce the oversized bounty crown while retaining a clear ring and readable name/health display.
- [x] Integrate the three isolated commits and inspect their combined differences.
- [x] Update only necessary asset tags through the full imported module closure; retain bare settings imports.
