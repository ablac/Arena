...
These rules were preserved during the canonical entrypoint migration. `AGENTS.md` remains authoritative when wording conflicts.
...
## Central public usernames

Read [public username contract](../../docs/public-usernames.md) before changing identity, public profile or chat behavior. `public_username` is nullable and separate from private `display_name`; OIDC sign-in is the only alias ingestion path. Never restore a real-name/email alias fallback or local username edits. Historical chat labels resolve the current alias by stable account ID, and alias refresh must preserve existing session-bound administrator grants.
